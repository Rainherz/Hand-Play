...
class VentanaMouse(QMainWindow):
    def __init__(self, parent=None):
        super(VentanaMouse, self).__init__(parent)

        # Carga la interfaz de usuario desde el archivo Mouse.ui
        current_dir = os.path.dirname(os.path.abspath(__file__))
        ui_file = os.path.join(current_dir, 'GUI', 'Mouse.ui')
        loadUi(ui_file, self)
        # Connect signals to slots
        self.home.clicked.connect(self.OpenHome)

        # Lista de juegos seleccionados
        selected_games = ['zuma', 'chess']
    ...
    def launch_game(self, path):
        ...
        self.process = Popen([sys.executable, os.path.join(script_directory, 'mouse.py')])
